...
gs.print('Change Request count = '+ chgReq.getRowCount());
    while (chgReq.next()) {
        var chgReqNum = chgReq.getValue('sys_id') ;
       gs.print(chgReqNum + ' Change State before = '+ chgReq.state);
       var getComment = new GlideRecord('sys_journal_field')
            getComment.addQuery('change_request.number',chgReqNum);
           // getComment.addQuery('sys_id','Wrire down the sysid of record of worknote');
            getComment.query();
            while(getComment.next()){
            getComment.deleteRecord();
            }
